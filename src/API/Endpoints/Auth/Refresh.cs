    ...
    .WithResponse<IResponse<AuthenticateResponse>>
{
    ...
    [HttpPost, SwaggerOperation(Description = "Refreshes token",
    ...
    public override async Task<ActionResult<IResponse<AuthenticateResponse>>> HandleAsync(
        [FromBody, SwaggerRequestBody("Refresh token payload")]
        RefreshRequest refreshRequest,
        CancellationToken cancellationToken = new()) =>
        Ok(await _mediator.Send(new RefreshCommand(refreshRequest), cancellationToken));
}