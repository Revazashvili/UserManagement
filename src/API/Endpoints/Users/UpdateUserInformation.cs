    ...
    .WithResponse<bool>
{
    ...
    [HttpPut, SwaggerOperation(Description = "Update user additional information",
    ...
    public override async Task<ActionResult<bool>> HandleAsync(
        [FromBody, SwaggerRequestBody("Update user information payload")]
        UpdateUserInformationRequest updateUserInformationRequest,
        CancellationToken cancellationToken = new()) =>
        Ok(await _mediator.Send(new UpdateUserInformationCommand(updateUserInformationRequest),
            cancellationToken));
}